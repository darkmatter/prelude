...
#
# `packages.prelude-prompt` is a starship.toml themed from the palette. Starship
# re-resolves $STARSHIP_CONFIG on every prompt render and direnv propagates
# env vars, so the devshell only needs `packages.prelude-shell` plus:
#
#   shellHook = ''export STARSHIP_CONFIG=${config.packages.prelude-prompt}'';
#
# The shell-core package supplies Starship, ble.sh, and bash-completion when
...
in {
  options.prelude.prompt = {
    enable = lib.mkEnableOption "themed starship prompt config (`packages.prelude-prompt` = starship.toml)";
