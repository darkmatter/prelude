...
config: let
  ...
  nestPath = path: value: lib.foldr (seg: acc: {${seg} = acc;}) value path;

  isPlaceholder = seg: seg == "*" || (lib.hasPrefix "<" seg && lib.hasSuffix ">" seg);
    ...
    go = prefix: attrs: segs:
      ...
      in
        ...
        in
          ...
          then cur
          else if lib.isOption cur && ((cur.type.getSubOptions or null) != null)
          then go nextPrefix (removeAttrs (cur.type.getSubOptions nextPrefix) ["_module"]) rest
          else if (!(lib.isOption cur)) && builtins.isAttrs cur
          then go nextPrefix (removeAttrs cur ["_module"]) rest
          else null;
  in
    go [] opts path;

  # Single-option leaf. lookupLoc is always the *raw* path.
  optionLeaf = title: lookupLoc: let
    val = optionAt lookupLoc;
    doc = nixosOptionsDoc (
      nixosOptionsArgs
      // {
        options = assert lib.assertMsg (val != null) ''
  ...
  treeFromDocList = let
    ...
    in
      if node.isLeaf && childNames == []
      then optionLeaf node.entry.displayName node.entry.lookupLoc
      else {
        kind = "group";
        title = name;
        children = childNodes;
  ...
  expandGenerate = node: let
    title =
      if (node.title or null) != null
      then node.title
      else "Options";
  ...
  expandNode = node:
    ...
    '';
      if (node.generate or null) != null
      then expandGenerate node
      else if (node.children or []) != []
      then let
        ...
        kids' =
          if (node.text or null) != null && kids != []
          then let
