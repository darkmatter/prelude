...
{lib}: let
  ...
  resolvePalette = theme: overrides:
  ...
  colorToRGB = value: let
    raw =
      if builtins.isInt value || builtins.isString value
      then toString value
      else throw "prelude: color must be a hex string or unsigned integer";
    decimal =
      if builtins.isInt value
      then value
      else if builtins.match "^[0-9]+$" raw != null
      then lib.toInt raw
      else null;
    ansiBase = [
      {
    ...
    ansiToRGB = index:
      if index < 16
      then builtins.elemAt ansiBase index
      else if index < 232
      then let
        offset = index - 16;
    ...
    hexToRGB = hex: let
      expanded =
        if builtins.stringLength hex == 4
        then "#${builtins.substring 1 1 hex}${builtins.substring 1 1 hex}${builtins.substring 2 1 hex}${builtins.substring 2 1 hex}${builtins.substring 3 1 hex}${builtins.substring 3 1 hex}"
        else hex;
    in {
  ...
  in
    ...
    then
      assert lib.assertMsg (decimal >= 0) "prelude: color index must be unsigned";
        ansiToRGB decimal
    else if builtins.match "^#[0-9A-Fa-f]{3}$" raw != null || builtins.match "^#[0-9A-Fa-f]{6}$" raw != null
    then hexToRGB raw
    else throw "prelude: unsupported color \"${raw}\"";

  rgbToHex = rgb: let
    channel = value: lib.fixedWidthString 2 "0" (lib.toLower (lib.toHexString value));
  in "#${channel rgb.r}${channel rgb.g}${channel rgb.b}";
  ...
  in {
    inherit palette;
    shadow = lightenColor palette.bg;
  };
  ...
  in {
    top =
      if (s.top or null) != null
      then s.top
      else (s.y or 0);
    bottom =
      if (s.bottom or null) != null
      then s.bottom
      else (s.y or 0);
    left =
      if (s.left or null) != null
      then s.left
      else (s.x or 0);
    right =
      if (s.right or null) != null
      then s.right
      else (s.x or 0);
  ...
  in
    ...
    then {
      ...
      comment =
        ...
        then lib.removePrefix "# " trimmed
        else lib.removePrefix "#" trimmed;
    }
    else {
      command = trimmed;
      comment = "";
    ...
    map (
      {
        ...
        steps =
          if explicitSteps != []
          then map normalizeRecipeStep explicitSteps
          else builtins.filter (s: s != null) (map lineToStep legacyLines);
      in {
        title =
          if title == null
          then name
  ...
  mdSplit = src: let
    ...
    srcPath =
      ...
      then src
      else if isAbsPathString src && builtins.pathExists src
      then src
      else null;

    markdown =
      if srcPath != null
      then builtins.readFile srcPath
      else if builtins.isString src
      then src
      else throw "prelude.lib.mdSplit: expected a path, path string, or markdown string";
    ...
    in
      if n > 0 && builtins.elemAt lines (n - 1) == ""
      then lib.take (n - 1) lines
      else lines;
    ...
    h2Title = line: let
      stripped =
        ...
        then builtins.substring 3 (builtins.stringLength line - 3) line
        else if lib.hasPrefix "##" line
        then builtins.substring 2 (builtins.stringLength line - 2) line
        else line;
    in
      lib.trim stripped;
    ...
    h1Title = line:
      if lib.hasPrefix "# " line
      then lib.trim (builtins.substring 2 (builtins.stringLength line - 2) line)
      else "";
    ...
    step = acc: line:
      ...
      else if !(acc.inFence or false) && isH2 line
      then {
        inFence = false;
        title = h2Title line;
        lines = [];
    ...
    dropLeadingBlanks = xs:
      if xs == []
      then []
      else if lib.trim (builtins.head xs) == ""
      then dropLeadingBlanks (builtins.tail xs)
      else xs;
    ...
    stripFirstH1 = ls: let
      go = inFence: acc: rest:
        ...
        in
          if (!inFence) && isH1 line
          then acc ++ tail
          else go nowIn (acc ++ [line]) tail;
    in
      go false [] ls;

    firstNonFencedH1 = ls: let
      go = inFence: rest:
        if rest == []
        then null
        else let
    ...
    slug = index: title: let
      base =
        if title == null || title == ""
        then "section"
        else title;
      safe = lib.strings.sanitizeDerivationName base;
    in "mdsplit-${toString index}-${safe}.md";
    ...
    text =
      if srcPath != null
      then srcPath
      else builtins.toFile "mdsplit-source.md" normalized;
  in {
    title = "README";
