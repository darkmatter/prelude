{lib}: {
  ...
  selectedExecutable =
    ...
    else lib.getExe' package program;
  exec =
    if command != null
    then command
    else lib.concatMapStringsSep " " lib.escapeShellArg ([selectedExecutable] ++ arguments);
  canonicalInvocation =
    if invocation != null
    then invocation
    else if command != null
    then command
    else lib.concatMapStringsSep " " lib.escapeShellArg ([(baseNameOf selectedExecutable)] ++ arguments);
in
