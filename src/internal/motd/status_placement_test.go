...
func TestStatusRendersInFooterForEveryTitleVariant(t *testing.T) {
	for _, test := range []struct {
		name       string
		title      string
		titleStyle string
	}{
		{name: "generated title", title: "ACME"},
		{name: "wordmark title", titleStyle: titleStyleSpine},
		{name: "inline title", titleStyle: titleStyleInline},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if statusIndex < 0 || linkIndex < 0 {
				t.Fatalf("footer is missing status or links:\n%s", footer)
			}
