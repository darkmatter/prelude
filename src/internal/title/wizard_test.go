...
func TestRenderWizardJSON(t *testing.T) {
	got := renderWizardJSON(wizardResult{
	...
	if cfg.Project != "acme-web" || cfg.Theme != "nord" || cfg.ColorProfile != "auto" {
		t.Fatalf("top-level fields wrong: %+v", cfg)
	}
	if len(cfg.Commands) != 2 || cfg.Commands[0].Exec != "pnpm dev" {
		t.Fatalf("commands wrong: %+v", cfg.Commands)
	}
	if cfg.Docs {
		t.Fatalf("docs should be disabled")
	}
}

func TestRenderWizardNixReadsSiblingJSON(t *testing.T) {
	got := renderWizardNix("prelude.json")

	for _, fragment := range []string{
		"wizard = builtins.fromJSON (builtins.readFile ./prelude.json);",
		...
		if !strings.Contains(got, fragment) {
			t.Fatalf("Nix wrapper missing fragment %q:\n%s", fragment, got)
		}
	}
}
...
func TestFinishWizardWritesTitleStarterDocsAndConfig(t *testing.T) {
	...
	jsonData, err := os.ReadFile("prelude.json")
	if err != nil {
		t.Fatalf("read prelude.json: %v", err)
	}
	var cfg WizardConfig
	if err := json.Unmarshal(jsonData, &cfg); err != nil {
		t.Fatalf("invalid prelude.json: %v\n%s", err, jsonData)
	}
	if cfg.Project != "acme" || !cfg.Docs || len(cfg.Commands) != 0 {
		t.Fatalf("prelude.json content wrong: %+v", cfg)
	}
	nixData, err := os.ReadFile("prelude.nix")
	if err != nil {
		t.Fatalf("read prelude.nix: %v", err)
	}
	if !strings.Contains(string(nixData), "./prelude.json") {
		t.Fatalf("prelude.nix does not reference JSON:\n%s", nixData)
	}
	if !strings.Contains(stderr.String(), "wrote title.txt\n") || !strings.Contains(stderr.String(), "wrote prelude.nix\n") || !strings.Contains(stderr.String(), "wrote prelude.json\n") {
...
func TestFinishWizardWritesTitleBesideNestedConfig(t *testing.T) {
	...
	jsonData, err := os.ReadFile("nix/prelude.json")
	if err != nil {
		t.Fatalf("read nix/prelude.json: %v", err)
	}
	if !strings.Contains(string(jsonData), `"project": "acme"`) {
		t.Fatalf("nested JSON missing project:\n%s", jsonData)
	}
	...
	if !strings.Contains(string(nixData), "./prelude.json") {
		t.Fatalf("nested config missing sibling JSON reference:\n%s", nixData)
	}
}
...
func TestJsonPathBesideConfig(t *testing.T) {
	cases := map[string]string{
		"prelude.nix":     "prelude.json",
		"./prelude.nix":   "prelude.json",
		"nix/prelude.nix": "nix/prelude.json",
	}
	for config, want := range cases {
		if got := jsonPathBesideConfig(config); got != want {
			t.Fatalf("jsonPathBesideConfig(%q) = %q, want %q", config, got, want)
		}
	}
}
