...
// in one pass. The finished result is printed to stdout as a ready-to-import
// flake-parts module; the interactive UI itself renders on stderr so stdout
// stays clean for redirection (`prelude-title --wizard > prelude.nix`).
...
// flake-parts was toggled off. titlePath is the path the rendered wordmark
// was written to, relative to where the config will live.
func renderWizardConfig(r wizardResult, titlePath string) string {
	if !r.FlakeParts {
		return renderStandaloneConfig(r, titlePath)
	}
	var b strings.Builder
	b.WriteString("# Generated by prelude-title --wizard.\n")
	b.WriteString("#\n")
	b.WriteString("# A flake-parts module: import it next to Prelude's module, e.g.\n")
	b.WriteString("#\n")
	b.WriteString("#   imports = [ prelude.flakeModules.default ./prelude.nix ];\n")
	b.WriteString("#\n")
	b.WriteString("{ ... }:\n{\n  prelude = {\n")
	fmt.Fprintf(&b, "    theme = %s;\n", nixString(r.Theme))
	fmt.Fprintf(&b, "    colorProfile = %s;\n", nixString(r.ColorProfile))
	fmt.Fprintf(&b, "    project = %s;\n\n", nixString(r.Project))
	if len(r.Commands) > 0 {
		b.WriteString("    commands = {\n")
		for _, command := range r.Commands {
			fmt.Fprintf(&b, "      %s = {\n", nixAttrKey(command.Name))
			// Exec stays omitted when empty: prelude infers it from the
			// parsed command name.
			if command.Exec != "" {
				fmt.Fprintf(&b, "        exec = %s;\n", nixString(command.Exec))
			}
			if command.Description != "" {
				fmt.Fprintf(&b, "        description = %s;\n", nixString(command.Description))
			}
			b.WriteString("      };\n")
		}
		b.WriteString("    };\n\n")
	} else {
		b.WriteString("    # Declare project commands for the menu and MOTD:\n")
		b.WriteString("    # commands.dev = { exec = \"pnpm dev\"; description = \"start the dev server\"; };\n\n")
	}
	if r.Motd {
		b.WriteString("    motd = {\n")
		...
		b.WriteString("      };\n")
		if len(r.Commands) > 0 {
			// Advertise the first few commands as MOTD next steps; the cap
			// keeps the banner scannable.
			names := make([]string, 0, 3)
			for _, command := range r.Commands[:min(len(r.Commands), 3)] {
				names = append(names, nixString(command.Name))
			}
			fmt.Fprintf(&b, "      commands = [ %s ];\n", strings.Join(names, " "))
		}
		b.WriteString("    };\n")
	} else {
		b.WriteString("    # MOTD disabled in the wizard; the rendered title is kept for later:\n")
		fmt.Fprintf(&b, "    # motd = { enable = true; title.text = %s; };\n", nixPath(titlePath))
	}
	fmt.Fprintf(&b, "    menu.enable = %t;\n", r.Menu)
	fmt.Fprintf(&b, "    prompt.enable = %t;\n\n", r.Prompt)
	if r.Docs {
		b.WriteString("    # Starter page written by the wizard; add more entries as docs grow.\n")
		b.WriteString("    docs.pages = [ { text = ./docs/getting-started.md; } ];\n")
	} else {
		b.WriteString("    # Markdown pages enable the docs viewer:\n")
		b.WriteString("    # docs.pages = [ { text = ./docs/getting-started.md; } ];\n")
	}
	b.WriteString("  };\n}\n")
	return b.String()
...
// renderStandaloneConfig emits prelude.lib builder calls for consumers that
// do not use flake-parts. The file is a function over the package set and the
// prelude flake input, returning one package per enabled component.
func renderStandaloneConfig(r wizardResult, titlePath string) string {
	var b strings.Builder
	b.WriteString("# Generated by prelude-title --wizard.\n")
	b.WriteString("#\n")
	b.WriteString("# Standalone prelude.lib builders (no flake-parts). Call with your\n")
	b.WriteString("# package set and the prelude flake input, e.g. in your flake.nix:\n")
	b.WriteString("#\n")
	b.WriteString("#   let ui = import ./prelude.nix { inherit pkgs prelude; }; in\n")
	b.WriteString("#   pkgs.mkShell { packages = [ ui.motd ui.menu ]; shellHook = \"motd\"; }\n")
	b.WriteString("#\n")
	b.WriteString("{ pkgs, prelude }:\n")
	b.WriteString("let\n")
	...
	b.WriteString("in\n{\n")
	if r.Motd {
		b.WriteString("  motd = prelude.lib.mkMotd\n")
		b.WriteString("    { inherit (pkgs) lib writeText buildGoModule; }\n")
		b.WriteString("    (shared // {\n")
		b.WriteString("      title = {\n")
		fmt.Fprintf(&b, "        text = %s;\n", nixPath(titlePath))
		b.WriteString("        align = \"center\";\n")
		b.WriteString("      };\n")
		b.WriteString("      commandCatalog = commands;\n")
		if len(r.Commands) > 0 {
			// Advertise the first few commands as MOTD next steps; the cap
			// keeps the banner scannable.
			names := make([]string, 0, 3)
		...
		b.WriteString("  # MOTD disabled in the wizard; the rendered title is kept for later:\n")
		fmt.Fprintf(&b, "  # motd = prelude.lib.mkMotd { … } (shared // { title.text = %s; });\n\n", nixPath(titlePath))
	}
	if r.Menu {
		if len(r.Commands) > 0 {
			b.WriteString("  menu = prelude.lib.mkMenu\n")
			b.WriteString("    { inherit (pkgs) lib writeShellApplication writeText buildGoModule; }\n")
			b.WriteString("    (shared // { inherit commands; });\n\n")
		} else {
			// mkMenu asserts a non-empty catalogue, so an empty `commands`
			// would fail the very first build. Ship it commented instead.
			b.WriteString("  # The menu needs at least one command: add entries to `commands`\n")
			b.WriteString("  # above, then uncomment:\n")
			b.WriteString("  # menu = prelude.lib.mkMenu\n")
			b.WriteString("  #   { inherit (pkgs) lib writeShellApplication writeText buildGoModule; }\n")
			b.WriteString("  #   (shared // { inherit commands; });\n\n")
		}
	...
	if r.Docs {
		b.WriteString("  # Starter page written by the wizard; add more entries as docs grow.\n")
		b.WriteString("  docs = prelude.lib.mkDocs\n")
		b.WriteString("    { inherit (pkgs) lib writeText buildGoModule; }\n")
		b.WriteString("    (shared // { pages = [ { text = ./docs/getting-started.md; } ]; });\n\n")
	}
	if r.Prompt {
		b.WriteString("  # The themed starship prompt is only generated by the flake-parts\n")
		b.WriteString("  # module (prelude.prompt.enable); there is no standalone builder yet.\n")
	}
	b.WriteString("}\n")
	return b.String()
}
...
// starterDocsPath is both written by the wizard and referenced by the
// emitted config, so the generated module builds without manual steps.
const starterDocsPath = "docs/getting-started.md"

const starterDocsPage = `# Getting started

This page was created by ` + "`prelude-title --wizard`" + `.
