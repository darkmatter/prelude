...
// The generated prelude.nix is a full options template: wizard choices are
// active; every other supported option is present as a commented default so
// the file doubles as inline documentation.
...
func renderWizardConfig(r wizardResult, titlePath string) string {
	...
	if err := configTemplates.ExecuteTemplate(&buf, "flake_parts.nix.tmpl", data); err != nil {
		...
		panic(fmt.Sprintf("render setup config: %v", err))
	}
	return buf.String()
}
...
func renderStandaloneConfig(r wizardResult, titlePath string) string {
	...
	var buf bytes.Buffer
	if err := configTemplates.ExecuteTemplate(&buf, "standalone.nix.tmpl", data); err != nil {
		panic(fmt.Sprintf("render standalone setup config: %v", err))
	}
	return buf.String()
}
