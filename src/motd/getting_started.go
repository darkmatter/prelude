...
// renderGettingStarted unifies commands + recipes under one heading
// (playground StartHeadingBelow): glow divider → centered title → sub-labels.
func (r renderer) renderGettingStarted() []string {
	...
	var out []string
	out = append(out, r.contentDivider(), r.blankContentLine())
	out = append(out, r.centeredHeading(heading), r.blankContentLine())
