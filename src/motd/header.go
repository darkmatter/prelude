...
func (r renderer) renderHeader() string {
	if r.cfg.Title != "" {
		return join(r.renderGeneratedTitle()...)
	}
...
func (r renderer) renderHeaderDivider() string {
	if r.cfg.Title == "" && strings.ToLower(r.cfg.Header.TitleStyle) == titleStyleInline {
		return r.inlineTitleRule(r.cfg.Project)
	}
	return r.headerUnderline()
}

func (r renderer) renderGeneratedTitle() []string {
	lines := strings.Split(strings.TrimRight(r.cfg.Title, "\n"), "\n")
	out := make([]string, 0, len(lines)+1)
	for _, line := range lines {
		title := inline(r.st.headerAccent).Bold(true).Render(line)
		out = append(out, r.centerHeaderLine(title))
	}
	return out
}

// centerHeaderLine centers content on the header surface.
func (r renderer) centerHeaderLine(content string) string {
	w := lipgloss.Width(content)
	pad := max(r.cardWidth-w, 0)
	left := pad / 2
	right := pad - left
	if !r.st.headerTransparent {
		return r.st.headerFill.Render(strings.Repeat(" ", left)) + content + r.st.headerFill.Render(strings.Repeat(" ", right))
	}
	...
	for _, it := range items {
		...
		if compact {
			if status != "" {
				...
				chip = inline(dot).Render("● ") + inline(r.st.headerDim).Render(label)
			}
		} else {
			if label != "" {
			...
			if status != "" {
				...
				chip += inline(r.st.headerDim).Render("  ") + inline(dot).Render("●")
			}
		}
		if chip != "" {
...
func (r renderer) padContentLine(styled, align string) string {
	...
	if r.cfg.Background == "" {
		...
		case "right":
			text = strings.Repeat(" ", pad) + styled
		default:
