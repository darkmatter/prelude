    ...
    flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { pkgs, config, ... }: {
        devShells.default = pkgs.mkShell {
          packages = [ config.packages.motd config.packages.docs ];
          shellHook = "motd";
        };
      };
    };
...
`packages.motd` bundles the menu and every command wrapper, so one package on
PATH gives your team the whole surface. Package-backed commands —
